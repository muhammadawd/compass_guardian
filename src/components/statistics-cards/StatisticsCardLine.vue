      ...
      <div class="p-6 pb-0" :class="{'flex justify-between flex-row-reverse items-center': iconRight}">
        <div>
          <h2 class="mb-1 font-bold">
            ...
            {{ statistic }}</h2>
          <div class="text-bold mb-2">{{ statisticTitle }}</div>
        </div>
      </div>

<!--      <div class="line-area-chart" :id="chartData.id">-->
<!--        <vue-apex-charts ref="apexChart" :type="type" height=100 width='100%' :options="chartData.chartOptions"-->
<!--                         :series="chartData.series"></vue-apex-charts>-->
<!--      </div>-->
    </div>
  </vx-card>
</template>
