      ...
      <div class="vx-col w-full md:w-1/1 mb-base">
        <vx-card class="text-right">
          <vs-alert class="mb-4" color="dark">
            <span class="text-bold">{{$ml.get('edit_delivery')}}</span>
          </vs-alert>
  ...
  export default {
    ...
    methods: {
      ...
      findAdmin() {
        ...
        try {
          window.serviceAPI.API().get(window.serviceAPI.FIND_DELIVERY+ `/${id}`)
            .then((response) => {
              vm.$root.$children[0].$refs.loader.show_loader = false;
              response = response.data;
              if (response.status) {
                vm.dataModel =  response.data.delivery
                return
              }
            }).catch((error) => {
            vm.$root.$children[0].$refs.loader.show_loader = false;
      ...
      editAdmin() {
        ...
        try {
          window.serviceAPI.API().post(window.serviceAPI.EDIT_DELIVERY, request_data)
            .then((response) => {
              response = response.data;
              if (response.status) {
