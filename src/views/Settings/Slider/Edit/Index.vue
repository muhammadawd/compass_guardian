      ...
      <div class="vx-col w-full md:w-1/1 mb-base">
        <vx-card class="text-right">
          <vs-alert class="mb-4" color="dark">
            <span class="text-bold">{{$ml.get('edit_slider')}}</span>
          </vs-alert>
  ...
  export default {
    ...
    methods: {
      findSlider() {
        ...
        try {
          window.serviceAPI.API().get(window.serviceAPI.FIND_SLIDER + `/${id}`)
            .then((response) => {
              vm.$root.$children[0].$refs.loader.show_loader = false;
              response = response.data;
              if (response.status) {
                vm.dataModel = vm.prepareModel(response.data.slider)
                return
              }
            }).catch((error) => {
            vm.$root.$children[0].$refs.loader.show_loader = false;
      ...
      editCategory() {
        ...
        try {
          window.serviceAPI.API().post(window.serviceAPI.EDIT_SLIDER, form_data, {
            headers: {
