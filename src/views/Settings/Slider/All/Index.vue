    ...
    <div class="vx-row">
      <div class="vx-col md:w-1/2 mb-2">
        <vs-button color="primary" class="text-bold" type="filled" icon-pack="feather" icon="icon-plus"
                   @click="$router.push({name: 'settings_slider_add'})">
          {{$ml.get('add_new')}}
        </vs-button>
      </div>
    </div>
    ...
    <div class="vx-row">
      <div class="vx-col w-full md:w-1/4 sm:w-1/2 mb-base " v-for="item in filteredKB" :key="item.id"
        ...
        <vx-card class="text-center">
          ...
          <div class="cursor-pointer" @click="$router.push({name:'settings_slider_edit',params:{id:item.id}})">
...
<script>
  ...
  export default {
    ...
    computed: {
      filteredKB() {
        return _.filter(this.sliders, (item) => {
    ...
    methods: {
      getAllSlider() {
        ...
        try {
          window.serviceAPI.API().get(window.serviceAPI.ALL_SLIDER)
            .then((response) => {
              vm.$root.$children[0].$refs.loader.show_loader = false;
              response = response.data;
              if (response.status) {
                vm.sliders = response.data.sliders.data;
      ...
      acceptAlert() {
        ...
        vm.$root.$children[0].$refs.loader.show_loader = true;
        try {
          window.serviceAPI.API().post(window.serviceAPI.DELETE_SLIDER + `/${id}`)
            .then((response) => {
              vm.$root.$children[0].$refs.loader.show_loader = false;
              response = response.data;
              if (response.status) {
