          ...
          <div class="vx-row">
            <div class="vx-col w-full text-center mb-base">
              <vs-button ref="loadableButton" id="button-with-loading" :disabled="loading"
                         ...
                         @click="editOptionValues" type="filled" vslor="primary">
                {{$ml.get('edit')}}
              </vs-button>
            </div>
          </div>
  ...
  export default {
    ...
    methods: {
      ...
      editOptionValues() {
        ...
        try {
          window.serviceAPI.API().post(window.serviceAPI.EDIT_OPTIONS, request_data)
            .then((response) => {
      ...
      findOptions() {
        ...
        try {
          window.serviceAPI.API().get(window.serviceAPI.FIND_OPTIONS+ `/${id}`)
            .then((response) => {
              vm.$root.$children[0].$refs.loader.show_loader = false;
              response = response.data;
              if (response.status) {
                vm.dataModel = response.data.option
                return
              }
            }).catch((error) => {
            vm.$root.$children[0].$refs.loader.show_loader = false;
