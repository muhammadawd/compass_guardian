          ...
          <div class="vx-row">
            <div class="vx-col w-full text-center mb-base">
              <vs-button ref="loadableButton" id="button-with-loading" :disabled="loading"
                         ...
                         @click="editMenuItem" type="filled" vslor="primary">
                {{$ml.get('edit')}}
              </vs-button>
            </div>
          </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      customLabel(o) {
        // return o.option_value ? o.option_value.translated : '';
        let label = `${o.translated ? o.translated.title : ''} ${o.unit ? o.unit.translated.title : ''}`;
        console.log(label)
        if (!(o.translated ? o.translated.title : '') && !(o.unit ? o.unit.translated.title : '')) {
          // alert('ss')
          label = `${o.option_value ? o.option_value.translated ? o.option_value.translated.title : '' : ''}
        ${o.option_value ? o.option_value.unit ? o.option_value.unit.translated.title : '' : ''}`;
        }
        return label;
      },
      handleFileUpload() {
      ...
      editMenuItem() {
        ...
        request_data.category_id = vm.dataModel.category ? vm.dataModel.category.id : '';
        ...
        try {
          window.serviceAPI.API().post(window.serviceAPI.EDIT_MENU, form_data, {
            headers: {
      ...
      getMenuItem() {
        ...
        vm.$root.$children[0].$refs.loader.show_loader = true;
        try {
          window.serviceAPI.API().get(window.serviceAPI.FIND_MENU + `/${vm.findId}`)
            .then((response) => {
              vm.$root.$children[0].$refs.loader.show_loader = false;
              response = response.data;
              console.log(response)
              if (response.status) {
                vm.dataModel = vm.prepareModelToShow(response.data.menu);
                vm.extraOptionsValues = response.data.menu.extra_options;
                return
              }
              vm.dataModel = {};
            }).catch((error) => {
      ...
      prepareModelToShow(model) {
        ...
        model.title_ar = arabic.length > 0 && arabic[0].title !== 'null' ? arabic[0].title : '';
        model.title_en = english.length > 0 && arabic[0].title !== 'null' ? english[0].title : '';
        model.description_ar = arabic.length > 0 && arabic[0].description !== 'null' ? arabic[0].description : '';
        model.description_en = arabic.length > 0 && arabic[0].description !== 'null' ? arabic[0].description : '';
        return model;
      },
      getAllExtraOptions() {
        let vm = this;
      ...
      prepareDataToSend(model) {
        ...
        $.each(model.option_values, (index, item) => {
          ...
          $.each(item.option_value_details, (key, value) => {
            form_data.append(`option_values[${index}][option_value_details][${key}][option_value_id]`, value.option_value_id ? value.option_value_id : value.id);
          })
        })
        return form_data;
      },
