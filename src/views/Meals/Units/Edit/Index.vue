      ...
      <div class="vx-col w-full md:w-1/1 mb-base">
        <vx-card class="text-right">
          <vs-alert class="mb-4" color="dark">
            <span class="text-bold">{{$ml.get('edit_unit')}}</span>
          </vs-alert>
  ...
  export default {
    ...
    methods: {
      findUnit() {
        ...
        try {
          window.serviceAPI.API().get(window.serviceAPI.FIND_UNITS + `/${id}`)
            .then((response) => {
              vm.$root.$children[0].$refs.loader.show_loader = false;
              response = response.data;
              if (response.status) {
                vm.dataModel = response.data.unit
                return
              }
            }).catch((error) => {
            vm.$root.$children[0].$refs.loader.show_loader = false;
      ...
      editCategory() {
        ...
        try {
          window.serviceAPI.API().post(window.serviceAPI.EDIT_UNITS, request_data)
            .then((response) => {
              response = response.data;
              if (response.status) {
                window.helper.showMessage('success', vm);
                vm.$router.push({name: 'meals_units'});
                return null;
              }
