...
<template>
  <div id="data-list-thumb-view" class="data-list-container">
    ...
    <vs-table ref="table" multiple v-model="selected" pagination :max-items="itemsPerPage" search :data="units">
...
<!--          </div>-->
        ...
        <vs-dropdown vs-trigger-click class="cursor-pointer mb-4 ml-4">
          <div dir="ltr"
            ...
            <span class="mr-2">{{ currentPage * itemsPerPage - (itemsPerPage - 1) }} - {{ units.length - currentPage * itemsPerPage > 0 ? currentPage * itemsPerPage : units.length }} of {{ units.length }}</span>
      ...
      <template slot-scope="{data}">
        ...
        <vs-tr :data="tr" :key="indextr" v-for="(tr, indextr) in data">
          ...
          <vs-td>
            <div class="btn-group">
              <vs-button @click="$router.push({name:'meals_units_edit',params:{id:tr.id}})" type="line"
                         color="primary">
  ...
  export default {
    ...
    methods: {
      ...
      acceptAlert() {
        ...
        console.log(ids)
        try {
          window.serviceAPI.API().post(window.serviceAPI.DELETE_UNITS, {
            ids: ids
          })
            .then((response) => {
              vm.$root.$children[0].$refs.loader.show_loader = false;
              response = response.data;
              if (response.status) {
                vm.units = window.helper.deleteMulti(ids, vm.units)
                // location.reload()
              }
            }).catch((error) => {
            vm.$root.$children[0].$refs.loader.show_loader = false;
