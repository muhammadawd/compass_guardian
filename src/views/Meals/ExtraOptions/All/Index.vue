...
<template>
  <div id="data-list-thumb-view" class="data-list-container">
    ...
    <add-new-data-sidebar :isSidebarActive="addNewDataSidebar" :updateUnits="updateUnits"
                          @closeSidebar="addNewDataSidebar = false"/>

    <vs-table ref="table" multiple v-model="selected" pagination :max-items="itemsPerPage" search :data="extraOptions">
...
<!--          </div>-->
        ...
        <vs-dropdown vs-trigger-click class="cursor-pointer mb-4 ml-4">
          <div dir="ltr"
            ...
            <span class="mr-2">{{ currentPage * itemsPerPage - (itemsPerPage - 1) }} - {{ extraOptions.length - currentPage * itemsPerPage > 0 ? currentPage * itemsPerPage : extraOptions.length }} of {{ extraOptions.length }}</span>
      ...
      <template slot="thead">
        ...
        <vs-th sort-key="name">{{$ml.get('price')}}</vs-th>
      ...
      <template slot-scope="{data}">
        ...
        <vs-tr :data="tr" :key="indextr" v-for="(tr, indextr) in data">
          ...
          <vs-td>
            <div class="btn-group">
              <vs-button @click="$router.push({name:'meals_extra_options_edit',params:{id:tr.id}})" type="line"
                         color="primary">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      updateUnits(unit) {
        this.extraOptions.unshift(unit)
      },
      deleteSelected() {
        this.$vs.dialog({
          type: 'confirm',
      ...
      acceptAlert() {
        ...
        console.log(ids)
        try {
          window.serviceAPI.API().post(window.serviceAPI.DELETE_EXTRA_OPTIONS, {
            ids: ids
          })
            .then((response) => {
              vm.$root.$children[0].$refs.loader.show_loader = false;
              response = response.data;
              if (response.status) {
                vm.extraOptions = window.helper.deleteMulti(ids, vm.extraOptions)
                // location.reload()
              }
            }).catch((error) => {
            vm.$root.$children[0].$refs.loader.show_loader = false;
