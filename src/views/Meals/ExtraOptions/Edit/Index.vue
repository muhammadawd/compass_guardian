      ...
      <div class="vx-col w-full md:w-1/1 mb-base">
        <vx-card class="text-right">
          <vs-alert class="mb-4" color="dark">
            <span class="text-bold">{{$ml.get('edit_extra_option')}}</span>
          </vs-alert>
          ...
          <div class="vx-row">
            ...
            <div class="vx-col md:w-1/4 mb-base">
              <vs-input class="w-full" :label="$ml.get('price')" v-model="dataModel.price"/>
              <span class="span-text-validation text-danger text-bold" id="price_error"></span>
            </div>
            <div class="vx-col md:w-1/2 mb-base">
              <vs-input class="w-full" :label="$ml.get('description_ar')" v-model="dataModel.description_ar"/>
              <span class="span-text-validation text-danger text-bold" id="description_ar_error"></span>
            </div>
            <div class="vx-col md:w-1/2 mb-base">
              <vs-input class="w-full" :label="$ml.get('description_en')" v-model="dataModel.description_en"/>
              <span class="span-text-validation text-danger text-bold" id="description_en_error"></span>
            </div>
          </div>
          <div class="vx-row">
            <div class="vx-col w-full text-center mb-base">
              <vs-button ref="loadableButton" id="button-with-loading" :disabled="loading" @click="editCategory()"
  ...
  export default {
    ...
    methods: {
      findUnit() {
        ...
        try {
          window.serviceAPI.API().get(window.serviceAPI.FIND_EXTRA_OPTIONS + `/${id}`)
            .then((response) => {
              vm.$root.$children[0].$refs.loader.show_loader = false;
              response = response.data;
              if (response.status) {
                vm.dataModel = response.data.extraOption
                return
              }
            }).catch((error) => {
            vm.$root.$children[0].$refs.loader.show_loader = false;
      ...
      editCategory() {
        ...
        try {
          window.serviceAPI.API().post(window.serviceAPI.EDIT_EXTRA_OPTIONS, request_data)
            .then((response) => {
              response = response.data;
              if (response.status) {
                window.helper.showMessage('success', vm);
                vm.$router.push({name: 'meals_extra_options'});
                return null;
              }
