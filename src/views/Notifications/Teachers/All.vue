  ...
  <div id="data-list-thumb-view" class="data-list-container">
    ...
    <div class="vx-row">
      <div class="vx-col w-full">
            ...
            <div slot="header" class="flex flex-wrap-reverse items-center flex-grow justify-between">
              ...
              <div class="flex flex-wrap-reverse items-center">
                ...
                <vs-button color="primary" class="text-bold" type="filled" icon-pack="feather" icon="icon-plus"
                           v-if="hasAccessPermission('create-teacher-alert')"
                           @click="$router.push({name: 'teacher_notifications_add'})">
                  {{$ml.get('add_new')}}
                </vs-button>
              </div>
              ...
              <vs-dropdown vs-trigger-click class="cursor-pointer mb-4 ml-4">
                <div dir="ltr"
                  ...
                  <span class="mr-2">{{ currentPage * itemsPerPage - (itemsPerPage - 1) }} - {{ notifications.length - currentPage * itemsPerPage > 0 ? currentPage * itemsPerPage : notifications.length }} of {{ notifications.length }}</span>
                ...
                <vs-td class="text-right">
                  <div class="btn-group" dir="ltr">
                    <vs-button @click="deleteSingle(tr.id)" type="line"
                               v-if="hasAccessPermission('delete-teacher-alert')"
                               color="danger">
        ...
        <vs-button @click="deleteSelected()" class="mt-4" v-if="hasAccessPermission('delete-teacher-alert')"
  ...
  export default {
    ...
    methods: {
      ...
      getAllNotifications() {
        ...
        try {
          window.serviceAPI.API().get(window.serviceAPI.ALL_TEACHER_NOTIFICATIONS)
            .then((response) => {
              vm.$root.$children[0].$refs.loader.show_loader = false;
              response = response.data;
              console.log(response)
              if (response.status) {
                vm.notifications = response.data.teacherAlerts.data;
                return
              }
              vm.notifications = [];
            }).catch((error) => {
