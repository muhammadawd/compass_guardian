  ...
  export default {
    ...
    methods: {
      ...
      findScientific() {
        ...
        vm.$root.$children[0].$refs.loader.show_loader = true;
        try {
          window.serviceAPI.API().get(window.serviceAPI.FIND_SCIENTIFIC_ATTACHMENT + `/${id}`)
            .then((response) => {
              vm.$root.$children[0].$refs.loader.show_loader = false;
              response = response.data;
              if (response.status) {
                vm.dataModel = response.data.scientificAttachment;
                vm.selectedStage = response.data.scientificAttachment.stage;
                vm.selectedSubjects = response.data.scientificAttachment.subject;
                vm.selectedClassRooms = response.data.scientificAttachment.class_room;
                return
              }
            }).catch((error) => {
            vm.$root.$children[0].$refs.loader.show_loader = false;
      ...
      addMagazine() {
        ...
        try {
          window.serviceAPI.API().post(window.serviceAPI.EDIT_SCIENTIFIC_ATTACHMENT, form_data, {
            headers: {
