      ...
      <div class="vx-col w-full">
        <vx-card class="text-center cursor-pointer">
          ...
          <vs-table ref="table" multiple v-model="selected" pagination :max-items="itemsPerPage" search
            ...
            <template slot="thead">
              <vs-th width="15%">{{$ml.get('name')}}</vs-th>
...
<script>
  ...
  export default {
    ...
    methods: {
      getAllTeachersComplaints() {
        let vm = this;
        vm.$root.$children[0].$refs.loader.show_loader = true;
        try {
          window.serviceAPI.API().get(window.serviceAPI.ALL_TEACHERS_COMPLAINTS)
            .then((response) => {
