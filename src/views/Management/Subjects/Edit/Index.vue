      ...
      <div class="vx-col w-full md:w-1/1 mb-base">
        <vx-card class="text-right">
          <vs-alert class="mb-4" color="dark">
            <span class="text-bold">{{$ml.get('edit_subject')}}</span>
          </vs-alert>
  ...
  export default {
    ...
    methods: {
        ...
        try {
          window.serviceAPI.API().get(window.serviceAPI.FIND_SUBJECTS+ `/${id}`)
            .then((response) => {
              ...
              if (response.status) {
                vm.dataModel = response.data.subject
                return
              }
            }).catch((error) => {
            vm.$root.$children[0].$refs.loader.show_loader = false;
      ...
      editSubject() {
        ...
        try {
          window.serviceAPI.API().post(window.serviceAPI.EDIT_SUBJECTS, request_data)
            .then((response) => {
              response = response.data;
              if (response.status) {
                window.helper.showMessage('success', vm);
                vm.$router.push({name: 'management_subjects'});
                return null;
              }
