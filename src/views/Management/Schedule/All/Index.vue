      ...
      <div class="vx-col w-full">
        <vx-card class="text-center cursor-pointer">
          ...
          <vs-table ref="table" multiple v-model="selected" pagination :max-items="itemsPerPage" search
            ...
            <div slot="header" class="flex flex-wrap-reverse items-center flex-grow justify-between">
              ...
              <vs-dropdown vs-trigger-click class="cursor-pointer mb-4 ml-4">
                <div dir="ltr"
                  ...
                  <span class="mr-2">{{ currentPage * itemsPerPage - (itemsPerPage - 1) }} - {{ schedules.length - currentPage * itemsPerPage > 0 ? currentPage * itemsPerPage : schedules.length }} of {{ schedules.length }}</span>
            ...
            <template slot="thead">
              <vs-th width="40%">{{$ml.get('name')}}</vs-th>
              <vs-th width="40%">{{$ml.get('class_room')}}</vs-th>
              <vs-th></vs-th>
            </template>
            ...
            <template slot-scope="{data}">
              <vs-tr :data="tr" :key="indextr" v-for="(tr, indextr) in data">
                ...
                <vs-td class="text-right">
                  <slot v-if="tr.class_room">
                    {{tr.class_room.translated.title}}
                  </slot>
                </vs-td>
                <vs-td class="text-right">
        ...
        try {
          window.serviceAPI.API().get(window.serviceAPI.ALL_SCHEDULE, {
            ...
            .then((response) => {
              ...
              console.log(response)
              if (response.status) {
                vm.schedules = response.data.schedules.data;
                return
              }
              vm.schedules = [];
            }).catch((error) => {
        ...
        try {
          window.serviceAPI.API().post(window.serviceAPI.DELETE_SCHEDULE, {
            ...
            .then((response) => {
              ...
              response = response.data;
              if (response.status) {
                vm.schedules = window.helper.deleteMulti(ids, vm.schedules)
                location.reload()
              }
            }).catch((error) => {
            vm.$root.$children[0].$refs.loader.show_loader = false;
